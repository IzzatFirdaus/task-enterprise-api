...
class PublicPageController extends Controller
{
    ...
    public function llms(): Response
    {
        $lines = [
            ...
            'Source: '.url('/'),
            '',
            'Enterprise Tasks is a Laravel 13 application for private personal task management with a separate, role-protected administrative workspace. Personal task data is owner-scoped; administrative mutations are auditable.',
            '',
            ...
            '',
            '## Authenticated application (not indexable)',
            '- Dashboard: '.route('dashboard'),
            '- Profile: '.route('profile.edit'),
            '- Personal tasks API: '.url('/api/tasks'),
            '',
            '## Administrative workspace (not indexable)',
            '- Admin dashboard: '.route('admin.dashboard'),
            '- Admin tasks: '.route('admin.tasks.index'),
            '- Admin users: '.route('admin.users.index'),
            '- Admin settings: '.route('admin.settings.index'),
            '- Admin audit logs: '.route('admin.audit-logs.index'),
            '',
            '## Boundaries',
            '- Personal task reads and writes are scoped to the authenticated task owner.',
            '- Admin authorization is role-based and distinct from personal task flows.',
            '- Administrative mutations are recorded in audit_logs.',
            '- The role relationship is the canonical authorization source; the legacy is_admin flag is compatibility-only.',
            '',
            '## Data collected',
            'The application stores: name, email, password hash, role assignments, task content, and administrative audit logs.',
            'No advertising cookies are used. Analytics, if enabled, are loaded only after explicit consent.',
        ];
