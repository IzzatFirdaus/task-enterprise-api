<div class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
    <div class="mb-4 flex items-center justify-between border-b border-slate-100 pb-3">
        <h3 class="text-base font-semibold text-slate-900">Task Overview</h3>
        <span class="text-xs text-slate-400">Total: {{ $tasks->total() }}</span>
    </div>
    ...
    @if ($tasks->isEmpty())
        <div class="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h4 class="mt-4 text-sm font-semibold text-slate-900">No tasks found</h4>
            <p class="mt-1 text-xs text-slate-500 max-w-sm">No tasks match your selected filter. Create a new task or switch the status filter above.</p>
        </div>
    @else
                            ...
                            <td class="py-3.5 pl-4 align-top text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button
                                        ...
                                        class="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-700 shadow-2xs transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-cyan-500"
                                    ...
                                    <button
                                        ...
                                        class="inline-flex items-center gap-1 rounded-lg border border-rose-200/80 bg-white px-2.5 py-1.5 text-xs font-medium text-rose-600 shadow-2xs transition hover:border-rose-300 hover:bg-rose-50 hover:text-rose-700 focus:outline-none focus:ring-2 focus:ring-rose-500"
