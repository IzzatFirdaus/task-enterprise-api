<div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-xs">
    <div class="mb-5 flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-3">
        <h3 class="text-lg font-bold text-slate-950 dark:text-white">Create task</h3>
        <span class="text-sm font-semibold text-slate-600 dark:text-slate-400">Your workspace</span>
    </div>

    <form wire:submit="save" class="space-y-4">
        <!-- Title Input -->
        <div class="space-y-1.5">
            <label for="create_title" class="block text-sm font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">
            ...
            />
            @error('title')
                <p id="create_title_error" class="text-sm font-semibold text-rose-700 dark:text-rose-400" role="alert">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="space-y-1.5">
            <div class="flex items-center gap-2">
                <label for="create_description" class="block text-sm font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                    Description <span class="font-normal text-slate-500 dark:text-slate-400">(optional)</span>
                </label>
                <x-tooltip text="Optional context: acceptance criteria, links, or notes to remember when you return to this task." label="Description field help" id="create-description-tooltip" />
            </div>
            <textarea
            ...
            ></textarea>
            @error('description')
                <p id="create_description_error" class="text-sm font-semibold text-rose-700 dark:text-rose-400" role="alert">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="space-y-1.5">
            <div class="flex items-center gap-2">
                <label for="create_status" class="block text-sm font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                    Initial Status
                </label>
                <x-tooltip text="Pending = not started. In progress = actively being worked on. Completed = finished. You can change this later." label="Initial status help" id="create-status-tooltip" />
            </div>
            <select
                id="create_status"
                wire:model="status"
            ...
            @error('status')
                <p class="text-sm font-semibold text-rose-700 dark:text-rose-400" role="alert">{{ $message }}</p>
            @enderror
        </div>
