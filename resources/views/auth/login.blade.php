    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-4">
        ...
        <div class="space-y-1">
            <div class="flex items-center justify-between">
                ...
                @if (Route::has('password.request'))
                    <a class="inline-flex min-h-11 items-center text-xs font-semibold text-teal-700 dark:text-teal-300 hover:text-teal-800 dark:hover:text-teal-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-600" href="{{ route('password.request') }}">
                        {{ __('Forgot password?') }}
                    </a>
                @endif
            </div>
        ...
        <div class="flex items-center justify-between pt-1">
            <label for="remember_me" class="inline-flex min-h-11 items-center cursor-pointer">
                <input id="remember_me" type="checkbox" class="h-5 w-5 rounded border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-teal-700 dark:text-teal-500 focus:ring-teal-500" name="remember">
        ...
        <div class="border-t border-slate-100 dark:border-slate-700 pt-4 text-center space-y-2">
            <p class="text-xs text-slate-500 dark:text-slate-400">
                ...
                <a class="inline-flex min-h-11 items-center font-semibold text-teal-700 dark:text-teal-300 hover:text-teal-800 dark:hover:text-teal-200 ml-1" href="{{ route('register') }}">
                    Create an account
                </a>
            </p>
