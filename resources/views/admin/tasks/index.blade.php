    ...
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 p-6 shadow-sm">
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm" aria-label="Moderation tasks list">
                    <thead>
                        ...
                        @forelse ($tasks as $task)
                            <tr class="group transition hover:bg-slate-50/80 dark:hover:bg-slate-700/50">
                                ...
                                <td class="py-3.5 pr-4 align-middle whitespace-nowrap">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-xs text-slate-600 dark:text-slate-400">{{ $task->user?->email }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-3.5 px-4 align-middle">
                                    ...
                                    @if ($task->description)
                                        <div class="text-xs text-slate-600 dark:text-slate-400 line-clamp-1 mt-0.5">{{ $task->description }}</div>
                                    @endif
                                </td>
                                ...
                                <td class="py-3.5 pl-4 align-middle text-right whitespace-nowrap">
                                    <form method="POST" action="{{ route('admin.tasks.delete', $task) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this task?');">
                                        ...
                                        <button
                                            type="submit"
                                            class="inline-flex min-h-[44px] min-w-[44px] items-center justify-center gap-1 rounded-lg border border-rose-300 dark:border-rose-900 bg-rose-50 dark:bg-rose-950/50 px-3 py-2 text-xs font-semibold text-rose-800 dark:text-rose-300 shadow-2xs hover:bg-rose-100 dark:hover:bg-rose-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-600"
