...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col justify-between gap-4 rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm sm:flex-row sm:items-center">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-950">Administrative Audit Trails</h1>
                <p class="text-xs text-slate-500">Immutable record of security events, privilege updates, and account actions.</p>
            </div>
            <a
                href="{{ route('admin.audit-logs.export') }}"
                class="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-slate-900"
            >
                        ...
                        @forelse ($logs as $log)
                            <tr class="group transition hover:bg-slate-50/80">
                                ...
                                <td class="py-3.5 pr-4 align-top whitespace-nowrap">
                                    <div class="font-medium text-xs text-slate-900">{{ $log->admin?->name ?? 'Automated System' }}</div>
                                    <div class="text-[10px] text-slate-400">{{ $log->admin?->email ?? 'system@internal' }}</div>
                                </td>

                                <!-- Action Badge -->
                                <td class="py-3.5 px-4 align-top whitespace-nowrap">
                                    <span class="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-0.5 text-xs font-semibold text-slate-700">
                                        {{ ucfirst(str_replace('_', ' ', $log->action)) }}
                                    </span>
                                </td>

                                <!-- Model Type & ID -->
                                <td class="py-3.5 px-4 align-top whitespace-nowrap text-xs text-slate-700">
                                ...
                                <!-- IP Address -->
                                <td class="py-3.5 px-4 align-top whitespace-nowrap text-xs text-slate-500">
                                ...
                                <td class="py-3.5 px-4 align-top text-xs text-slate-600">
                                    @if (!empty($log->changes))
                                        <details class="group/details cursor-pointer">
                                            <summary class="text-xs font-semibold text-cyan-700 hover:text-cyan-800 focus:outline-none">
                                                View parameters
                                            </summary>
                                ...
                                <!-- Timestamp -->
                                <td class="py-3.5 pl-4 align-top text-right whitespace-nowrap text-xs text-slate-500">
